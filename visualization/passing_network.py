...
def draw_pitch(min_x=0, max_x=1, title="", legend=""):
    """
    ...
    ax.plot(52.5, 34, 'o', color='green', markersize=10, zorder=5, alpha=0.8)
...
def draw_dispersion_dot(ax, player_position, title="", legend=""):
    player_position["origin_pos_y"] = player_position["origin_pos_y"]*height
    player_position["origin_pos_x"] = player_position["origin_pos_x"]*width
    player_x_list = []
    player_y_list = []
    for player_name, row in player_position.iterrows():
        player_x = row["origin_pos_x"]
        player_x_list.append(player_x)
        player_y = row["origin_pos_y"]
        player_y_list.append(player_y)
        ax.plot(player_x, player_y, 'o', color='red', markersize=10, zorder=5, alpha=0.2)
    # get the mean of values in player_x_list and player_y_list
    player_x_mean = np.mean(player_x_list)
    player_y_mean = np.mean(player_y_list)
    dispersionIndex = (np.std(player_x_list) + np.std(player_y_list)) / 2
    # ax.annotate("Dispersion index: " + str(dispersionIndex), xy=(0.99*width, 0.02*height),
    #             ha="right", va="bottom", zorder=7, fontsize=10, color=config["lines_color"])
    left_right_index = player_y_mean - 34
    forward_backward_index = player_x_mean - 52.5
    # ax.annotate("Left-right index: " + str(left_right_index), xy=(0.99*width, 0.05*height),
    #             ha="right", va="bottom", zorder=7, fontsize=10, color=config["lines_color"])
    # ax.annotate("For-backward index: " + str(forward_backward_index), xy=(0.99*width, 0.08*height), ha="right", va="bottom", zorder=7, fontsize=10, color=config["lines_color"])
    ax.plot(player_x_mean, player_y_mean, 'o', color='blue', markersize=10, zorder=5, alpha=0.2)

    print("player_x_mean", player_x_mean)
    print("player_y_mean", player_y_mean)
    return ax, dispersionIndex, left_right_index, forward_backward_index, player_x_mean, player_y_mean

def draw_dispersion_dot_pass_value(ax, player_position, pair_pass_value, player_pass_count, player_pass_value, title="", legend="", max_pair_value=None):
    max_pair_value = pair_pass_value.pass_value.max() if max_pair_value is None else max_pair_value
    player_position["origin_pos_y"] = player_position["origin_pos_y"]*height
    player_position["origin_pos_x"] = player_position["origin_pos_x"]*width
    player_x_list = []
    ...
    for player_name, row in player_stats.iterrows():
        ...
        player_y_list.append(player_y)
        pass_value = row["pass_value"]
        pass_value_list.append(pass_value)
        edge_cmap = cm.get_cmap(config["nodes_cmap"])
        # edge_cmap = cm.get_cmap("nodes_cmap")
        norm = Normalize(vmin=0, vmax=max_pair_value)
        edge_color = edge_cmap(norm(pass_value))
        ax.plot(player_x, player_y, 'o', color=edge_color, markersize=10, zorder=5, alpha=0.2)
    # get the mean of values in player_x_list and player_y_list using pass_value_list as weights
    player_x_mean = np.average(player_x_list, weights=pass_value_list)
    player_y_mean = np.average(player_y_list, weights=pass_value_list)
    # player_x_mean = np.mean(player_x_list)
    # player_y_mean = np.mean(player_y_list)
    dispersionIndex = (np.std(player_x_list) + np.std(player_y_list)) / 2
    # ax.annotate("Dispersion index: " + str(dispersionIndex), xy=(0.99*width, 0.02*height),
