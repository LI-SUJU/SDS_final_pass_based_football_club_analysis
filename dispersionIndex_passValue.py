...
from visualization.passing_network import draw_pitch, draw_dispersion_dot_pass_value
import os
...
def draw_plot_for_1_match(match_id, team_name, ax):
    ...
    # ax = draw_pitch()
    ax, dispersionIndex, left_right_index, forward_backward_index, player_x_mean, player_y_mean = draw_dispersion_dot_pass_value(ax, player_position, pair_pass_value, player_pass_count, player_pass_value)
...
def draw_plot_for_1_club(team_id, team_name, matches_path):
    ...
    # save all the values in a json file
    data = {team_name:{
        "average_dispersionIndex": average_dispersionIndex,
        "average_left_right_index": average_left_right_index,
        "average_forward_backward_index": average_forward_backward_index,
        "average_player_x_mean": average_player_x_mean,
        "average_player_y_mean": average_player_y_mean
    }}
    with open("generated_data/spain/data.json", 'a') as file:
    ...
    # save_path = "plots/dispersionIndex_passValue_England(2015-2016)"
    # if not os.path.exists(save_path):
    #     os.makedirs(save_path)
    # plt.savefig("{0}/{1}.png".format(save_path, team_name))
...
France = 7
Spain = 11
path = "data/eventing/matches/{0}/27.json".format(Spain)
